...
fn main() -> Result<()> {
    ...
    app.lines = {
        ...
        let mut visible_layers = BTreeSet::<&str>::new();

        for l in drawing.layers() {
            if l.is_layer_on {
                visible_layers.insert(l.name.as_str());
            }
        }

        let mut blocks = BTreeMap::<&str, SmallVec<[AnyShape; 4]>>::new();

        // FIXME: Does not handle `base_point` which may change the 0 point
        for b in drawing.blocks() {
            let mut lines = SmallVec::<[AnyShape; 4]>::new();
            ...
            blocks.insert(b.name.as_str(), lines);
        }
